...
import jp.co.yahoo.adsdisplayapi.v11.model.CampaignServiceAppOsType;
import jp.co.yahoo.adsdisplayapi.v11.model.CampaignServiceBiddingStrategyConfiguration;
import jp.co.yahoo.adsdisplayapi.v11.model.CampaignServiceBudget;
import jp.co.yahoo.adsdisplayapi.v11.model.CampaignServiceConversionOptimizer;
import jp.co.yahoo.adsdisplayapi.v11.model.CampaignServiceConversionTracker;
import jp.co.yahoo.adsdisplayapi.v11.model.CampaignServiceCustomParameters;
import jp.co.yahoo.adsdisplayapi.v11.model.CampaignServiceIsRemoveFlg;
import jp.co.yahoo.adsdisplayapi.v11.model.CampaignServiceLabel;
import jp.co.yahoo.adsdisplayapi.v11.model.CampaignServiceServingStatus;
import jp.co.yahoo.adsdisplayapi.v11.model.CampaignServiceUserStatus;
import jp.co.yahoo.adsdisplayapi.v11.model.CampaignServiceViewableFrequencyCap;
...
public class Campaign {
  ...
  **/
  ...
  public Campaign vendorName(String vendorName) {
   ...
   /**
   * &lt;div lang&#x3D;\&quot;ja\&quot;&gt; キャンペーンの広告効果測定ツールのベンダー名です。キャンペーン目的「アプリ訴求」の場合にのみ設定されます。&lt;br&gt; このフィールドはキャンペーン作成後、最初にリンク先URLを設定した広告が作成された時に設定されます。&lt;br&gt; キャンペーンに紐づく広告のリンク先URLには、このベンダー名に対応する広告効果測定ツールのURLのみが指定できます。&lt;br&gt; vendorNameをNONEに設定するためには、SKAdNetwork計測による広告配信の最適化をOFFに設定する必要があります。SKAdNetwork計測による広告配信の最適化の変更は広告管理ツールからのみ変更できます。&lt;br&gt; ADD時およびSET時、&lt;code&gt;APPSFLYER&lt;/code&gt;, &lt;code&gt;ADJUST&lt;/code&gt;, &lt;code&gt;SINGULAR&lt;/code&gt;, &lt;code&gt;KOCHAVA&lt;/code&gt; および &lt;code&gt;NONE&lt;/code&gt; のみ指定可能です。 &lt;/div&gt; &lt;div lang&#x3D;\&quot;en\&quot;&gt; Vendor name of tracking tool for the campaign. It will be set only when the campaign goal is \&quot;app promotion\&quot;. &lt;br&gt; This field will be set when the first ad with destination URL is created after creating the campaign.&lt;br&gt; Only the tracking tool URL corresponds to this tool vendor name can be specified for the destination URL of the ad linked to the campaign.&lt;br&gt; To set vendorName to NONE, you must turn off optimization for ad delivery by SKAdNetwork tracking. The optimization of ad delivery by SKAdNetwork tracking can be set by the Campaign Management Tool.&lt;br&gt; Only &lt;code&gt;APPSFLYER&lt;/code&gt;, &lt;code&gt;ADJUST&lt;/code&gt;, &lt;code&gt;SINGULAR&lt;/code&gt;, &lt;code&gt;KOCHAVA&lt;/code&gt; and &lt;code&gt;NONE&lt;/code&gt; can be specified in ADD and SET operation. &lt;/div&gt; 
   * @return vendorName
  **/
  @javax.annotation.Nullable
  @ApiModelProperty(value = "<div lang=\"ja\"> キャンペーンの広告効果測定ツールのベンダー名です。キャンペーン目的「アプリ訴求」の場合にのみ設定されます。<br> このフィールドはキャンペーン作成後、最初にリンク先URLを設定した広告が作成された時に設定されます。<br> キャンペーンに紐づく広告のリンク先URLには、このベンダー名に対応する広告効果測定ツールのURLのみが指定できます。<br> vendorNameをNONEに設定するためには、SKAdNetwork計測による広告配信の最適化をOFFに設定する必要があります。SKAdNetwork計測による広告配信の最適化の変更は広告管理ツールからのみ変更できます。<br> ADD時およびSET時、<code>APPSFLYER</code>, <code>ADJUST</code>, <code>SINGULAR</code>, <code>KOCHAVA</code> および <code>NONE</code> のみ指定可能です。 </div> <div lang=\"en\"> Vendor name of tracking tool for the campaign. It will be set only when the campaign goal is \"app promotion\". <br> This field will be set when the first ad with destination URL is created after creating the campaign.<br> Only the tracking tool URL corresponds to this tool vendor name can be specified for the destination URL of the ad linked to the campaign.<br> To set vendorName to NONE, you must turn off optimization for ad delivery by SKAdNetwork tracking. The optimization of ad delivery by SKAdNetwork tracking can be set by the Campaign Management Tool.<br> Only <code>APPSFLYER</code>, <code>ADJUST</code>, <code>SINGULAR</code>, <code>KOCHAVA</code> and <code>NONE</code> can be specified in ADD and SET operation. </div> ")
